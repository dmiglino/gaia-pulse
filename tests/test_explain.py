...
class TestNoFixedRationalesLeft:
    """Que ningún `rationale` de comida o actividad vuelva a ser una cadena literal.

    Sirve para lo que un test de comportamiento no alcanza: una frase fija nueva pasa todos
    los tests de generador porque el campo sigue siendo un `str`. `blood_generator` y
    `pantry_generator` quedan afuera a propósito — sus cadenas son la 4.5.6 y la 4.5.7, y
    listarlas acá haría fallar el test por trabajo que todavía no toca.
    """

    _GENERATORS = ("meal_generator.py", "activity_generator.py")
