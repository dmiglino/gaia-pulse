...
class TestMuscleGroups:
    ...
    def test_muscle_groups_aggregated(
        self, db: Session, diego: User, household: Household
    ) -> None:
...
class TestActiveDays:
    ...
    def test_inactive_days_not_active(
        self, db: Session, diego: User, household: Household
    ) -> None:
        svc = DashboardService(db)
        data = svc._active_days_data(diego.id, household.id)
...
class TestPantrySummary:
    ...
    def test_pantry_with_items(
        self, db: Session, banana: FoodItem, household: Household
    ) -> None:
        stock = PantryStock(
            household_id=household.id,
