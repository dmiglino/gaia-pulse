...
**7.9 — Mezclar temas en una sola frase (segmentar oraciones)** — cerrada:

- [x] Tests de segmentación escritos primero (rojo confirmado contra los dos ejemplos del
      hueco: `cené fideos y corrí 30 minutos` perdía las dos intenciones por las guardas de
      exclusión mutua de `parse()`; `compré leche y pesé 80 kg` metía un ítem de stock
      fantasma llamado "pesé 80 kg" porque `stock_add` escaneaba la frase entera sin cortar
      en el conector), corriendo `tests/test_nlp.py` completo después de cada paso interno
      (implementación inicial, después de sacar la deuda nueva de `ruff`/`mypy`), y
      `tests/` completo dos veces.
      `app/nlp/rules.py`: `_trigger_types(text)` reusa los siete regex de disparador que ya
      existían (`meal`/`workout`/`body_metric`/`stock_add`/`stock_consume`/`pref_neg`/
      `pref_pos`) para decir qué categorías dispara un fragmento; `_segment_topics(text)`
      corta solo en " y "/" and "/". "/"; " y solo cuando el fragmento de después dispara una
      categoría que el acumulado todavía no tenía —misma categoría, o ninguna, se re-funde
      con el conector y da idéntico al texto original—. La función que hacía todo el trabajo
      se renombra a `_parse_segment` (cuerpo sin tocar) y el nuevo `parse()` público delega
      directo a `_parse_segment` cuando hay un solo segmento: esto garantiza que ninguna de
      las ~40 aserciones en castellano existentes cambia de resultado, verificado corriendo
      la suite completa. Cuando hay más de un segmento, cada uno se parsea por separado y se
      combinan las intenciones reales (no `mixed`); si ningún segmento solo produjo nada, cae
      al mismo `_parse_segment(text, ...)` sobre el texto completo en vez de duplicar su
      lógica de fallback en un helper aparte —la primera versión sí duplicaba esa lógica y
      reintrodujo un `arg-type` de `mypy` ya conocido en un sitio nuevo; la corrección fue
      borrar el helper, no anotar el tipo—.
      Efecto secundario documentado y aceptado: como cada segmento se parsea solo, el
      `raw_span` de cada intent partido queda acotado a su propia cláusula en vez de a la
      frase completa —campo puramente cosmético del preview de confirmación
      (`preview_partial.html:214-226`), sin ningún consumidor que haga aritmética de
      posiciones sobre él—.
      `tests/test_nlp.py`: clase nueva `TestSegmentarTemasMezclados` con los dos ejemplos del
      hueco, el ejemplo que la sección ya documentaba como funcionando (`pesé 81 kg y dormí 7
      horas`, una sola medición) para no romperlo, y un caso de un solo tema para blindar el
      passthrough.
      **Revisión `code-health-qa` (APROBADA CON SEGUIMIENTO):** corrigió una línea de un test
      nuevo que no entraba en 100 columnas de `black`. Encontró y documentó, sin que esta fase
      lo hubiera pedido, que separar `pref_neg`/`pref_pos` en categorías distintas también
      arregla gratis `me gusta el pollo y odio el pescado` —antes daba una sola preferencia
      arruinada (`item_name="pollo pescado"`, `dislikes`, perdiendo el "me gusta el pollo"
      entero); ahora da las dos preferencias limpias—, mejora sin test que la blindara: se
      agregó `test_me_gusta_el_pollo_y_odio_el_pescado_da_dos_preferencias`. También señaló un
      comentario que afirmaba un orden de evaluación que no era cierto —corregido quitando esa
      cláusula— y un bug de **línea de base, no de esta fase**, verificado con `git stash`
      contra el árbol previo al cambio: `_PREFERENCE_NEG_BARE` (el "no"/"never" suelto como su
      propia categoría de disparador) ya generaba una preferencia basura sobre frases como
      "cenamos fideos y no quedamos satisfechos" desde antes de la 7.9 —esta fase solo le
      cambia la forma exacta a la basura, no la introduce—; queda afuera de este commit,
      pendiente como su propio hallazgo sobre `_parse_preference`.

**7.10 — Remember-me y recuperación de contraseña** — cerrada:

- [x] Remember-me: `login_submit` (`app/web/auth.py`) ya leía el checkbox `remember` del
      form pero lo ignoraba; ahora `resp.set_cookie(..., max_age=settings.session_max_age_seconds
      if remember else None)`. Sin `remember`, `max_age=None` deja la cookie de sesión
      (el navegador la borra al cerrarse); el token firmado sigue aceptando hasta
      `session_max_age_seconds` en `decode_session_token` de cualquier forma —el cambio
      solo decide cuánto vive la cookie en disco, no cuánto vale la firma—. Sin
      infraestructura nueva, como pedía el hueco.
- [x] Recuperación de contraseña real: `PUBLIC_BASE_URL` nuevo en `.env.example`/
      `app/core/config.py` (con qué se arman los links que salen del servidor —nunca con
      `request.base_url`, que viene del header `Host` y es controlable por quien manda el
      request, no por quien despliega la app—); `app/services/mail_service.py` nuevo
      (SMTP opcional: sin `SMTP_HOST` solo loguea, igual que `OPENAI_API_KEY`); migración
      `0007` y `app/models/password_reset_token.py` para la tabla de tokens con
      expiración (`RESET_TOKEN_TTL`), guardando solo el hash SHA-256 del token crudo
      (`_hash_token()` en `app/services/auth_service.py`), nunca el token en claro; las
      pantallas `auth/forgot_password.html` y `auth/reset_password.html`, antes
      inexistentes y apuntando a `href="#"`.
      `AuthService.request_password_reset()` devuelve la misma respuesta exista o no el
      email —ni excepción ni valor distinguible— para no regalar un oráculo de qué
      emails viven en la casa; `tests/test_web_auth.py` suma 14 tests nuevos
      (`test_login_without_remember_is_a_session_cookie`,
      `test_login_with_remember_is_a_persistent_cookie`,
      `test_forgot_password_same_response_for_unknown_email`,
      `test_forgot_password_creates_token_and_sends_mail_for_real_email`,
      `test_reset_password_page_expired_token_shows_invalid`,
      `test_reset_password_submit_updates_password`,
      `test_reset_password_token_cannot_be_reused`, entre otros) y
      `tests/test_web_routes.py` suma las dos exclusiones del smoke de páginas para
      `/forgot-password` y `/reset-password/{token}` (no autenticadas / necesitan un
      token propio).
      **Revisión `security-privacy` (primera pasada: BLOQUEADA; segunda: APROBADA CON
      SEGUIMIENTO):** encontró que `forgot_password_submit` armaba el link de reset con
      `str(request.base_url)` —el header `Host` es del atacante, no del deploy, así que
      un request con un `Host` falsificado hacía mandar un link de reset válido a un
      dominio ajeno—, corregido con el `public_base_url` fijo de arriba. Encontró también
      que un fallo de SMTP en `send_email` no estaba atrapado: con un email **real** pero
      el servidor de correo caído, la ruta tiraba una excepción sin manejar (500) mientras
      que un email **inexistente** siempre devolvía 200 sin llegar a este punto —esa
      asimetría de códigos de estado es un oráculo de qué emails existen, peor que
      cualquier diferencia de tiempo—, corregido envolviendo el `smtplib.SMTP(...)` en un
      `try/except Exception` que loguea y traga (no relogueado con datos sensibles: solo
      destinatario y asunto, nunca el token). Señaló además, como nit de orden, que
      `reset_password_submit` chequeaba `password != password_confirm` antes de validar
      el token, dejando confirmar (o no) que un token inválido igual "aceptaba" el POST
      hasta la comparación de contraseñas; reordenado para chequear el token primero. Deja
      como seguimiento no bloqueante, ya aceptado: el side-channel de tiempo entre email
      conocido/desconocido (LOW), que el reset no invalida otras sesiones activas (patrón
      ya existente en toda la app, no una regresión de esta fase), y que el TTL vive
      duplicado entre el código y el texto del mail.
      **Revisión `code-health-qa` (primera pasada: BLOQUEADA; segunda: APROBADA):**
      encontró que `request_password_reset`/`get_valid_reset_token` armaban un
      `select()`/`db.add()` crudo contra `PasswordResetToken` en vez de pasar por la capa
      de repositorios —viola la regla no negociable de `AGENTS.md` §2—, corregido con
      `PasswordResetTokenRepository` nuevo (`app/repositories/password_reset_repo.py`),
      que espeja la forma exacta de `UserRepository.get_by_email`; verificado por la
      re-revisión, vía grep, que no queda ninguna otra consulta cruda contra
      `PasswordResetToken` en `app/`. Señaló también, sin bloquear, que la property
      `email_enabled` (`bool(self.smtp_host)`) ya existía sin ningún llamador —corregido
      cableándola en el guard de `mail_service.send_email`, que antes miraba
      `settings.smtp_host` directo— y una asimetría con `authenticate()`: `reset_password`
      no chequeaba `user.is_active`, dejando resetear la contraseña de una cuenta
      desactivada; corregido agregando el mismo chequeo. Deja como seguimiento no
      bloqueante, ya aceptado: la falta de un mínimo de longitud/complejidad en la
      contraseña nueva (hueco compartido con el signup, no una regresión de esta fase) y
      la ubicación de `_hash_token` en `auth_service.py` en vez de `app/core/security.py`.
      Ambas correcciones se reverificaron con la suite completa dos veces (antes y después
      de las dos rondas de re-revisión) sin ninguna deuda nueva de `ruff`/`black`/`mypy`.
