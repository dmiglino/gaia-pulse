...
Four generators produce raw suggestion dicts:
...
- **`blood_generator`** — reads the latest panel, but reads its **age** first: fresh advises and names the panel's date, stale advises with a caveat and a reduced confidence, and past a year — or with no readable date at all — it emits no advice, only one "repeat the panel" card that says how many markers went unread and never which. Each entry names a food or a movement and the mechanism that ties it to the marker; the not-a-diagnosis framing is written once per screen, not once per card
- **`pantry_generator`** — the only household-scoped generator: what read zero, what is closest to running out, what gets bought regularly and is not in stock, and what tends to be bought on the same day as something that is now at zero. Its four cards explain with the numbers they measured — how many of the tracked items are at zero, which one has the least margin against its own threshold, how many times something was bought in the last 60 days. Low stock is `PantryStock.is_low`, the same rule the pantry grid reads. These cards are not the per-item alert: that is the `low_stock` notification, which retires when the item is restocked
...
- **Attribute generalization** — the same delta computed on the subject's **attribute** and then halved (`_ATTRIBUTE_SIGNAL_SCALE = 0.5`). This is what makes rejecting broccoli, cauliflower and kale say something about spinach. A category is one of the reasons something is liked, never the whole reason. There are **two** attribute vocabularies (`learning.ATTRIBUTE_TYPES`), one per catalogue: `food_category` from `FoodItem`, and `muscle_group` from `ExerciseType` — so rejecting "Bench Press" and "Push-ups" moves "Incline Press" the same way. Exercises only resolve from the catalogue side, because `ExerciseType` has no `aliases_json` and `FoodItem` does: a `repeated_activity` signal written in Spanish by a capture ("press de banca") does not fill the chest bucket.
...
- **A category has no button**, because forgetting it would have nothing to delete: the conclusion is derived from the catalogue on every read, over the signals of the items behind it. It goes away when those items do. For `food_category` that is also true of the storage — the type sits in `ATTRIBUTE_SUBJECT_TYPES` (`ATTRIBUTE_TYPES − SUBJECT_TYPES`), so `record_signal` rejects it and no row with that type can exist. `muscle_group` is the case that shows the two statements are not the same one: it *is* a subject type, a workout capture writes rows with it, and so it can appear **twice** in the panel — above as a subject with a Forget button, for what was trained, and here as a conclusion without one, for what was said about that group's exercises. Two different rows, and the type label next to each conclusion (`domain.html`'s `learned_attribute_kind_label`) is what keeps them from reading as a duplicate.
